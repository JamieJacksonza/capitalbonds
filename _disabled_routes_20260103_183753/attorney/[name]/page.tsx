...
function attorneysForDeal(d: any): string[] {
  const out: string[] = [];

  const reg = String(d?.registration_attorney ?? d?.registrationAttorney ?? "").trim();
  if (reg) out.push(reg);
...
export default function AttorneyDealsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    for (const d of list) {
      const names = attorneysForDeal(d).map(norm);
      if (!names.includes(who)) continue;
      if (!inRange(dealDate(d), from, to)) continue;
    ...
    return picked.sort((a: any, b: any) => {
    ...
    <div className="mx-auto w-full max-w-7xl px-7 py-10 md:px-12 lg:px-16">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="text-xs font-extrabold text-black/60">
            <Link href="/" className="hover:underline">Dashboard</Link> / Attorney / Firm
          </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-black/10 bg-white px-4 py-6 text-sm font-semibold text-black/60">
          No deals match this attorney/firm + date range.
        </div>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-black/[0.02]">
              <tr className="text-xs font-extrabold text-black">
                ...
                <th className="px-4 py-3">Registration Attorney</th>
            ...
            <tbody>
              {filtered.map((d: any) => (
                <tr key={d.id} className="border-t border-black/10 hover:bg-black/[0.02]">
                  ...
                  <td className="px-4 py-3">{String(d?.registration_attorney ?? "").trim() || ""}</td>
