...
export default function AgentPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return list
      .filter((d: any) => pickAgent(d) === agentName)
      .filter((d: any) => inRange(dealDate(d), from, to));
  }, [deals, agentName, from, to]);
  ...
  function applyFilter() {
    ...
    if (to) qs.set("to", to);
    const q = qs.toString();
    router.push(q ? `/agent/${encodeURIComponent(agentName)}?${q}` : `/agent/${encodeURIComponent(agentName)}`);
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-7 py-10 md:px-12 lg:px-16">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="text-2xl font-extrabold text-black">Agent: {agentName}</div>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-black/10 bg-white">
        {loading ? (
          ...
          <div className="p-6 text-sm font-semibold text-black/60">No deals for this agent in this range.</div>
        ) : (
