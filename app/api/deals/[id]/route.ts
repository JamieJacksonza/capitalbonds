...
import { getSessionFromRequest, requireAdmin } from "@/app/lib/auth";
...
export async function PATCH(req: Request) {
  ...
  const stageData = body?.stageData && typeof body.stageData === "object" ? body.stageData : null;
  ...
  const regPaidAtRaw =
    body?.registration_paid_at ??
    body?.registrationPaidAt ??
    stageData?.registration_paid_at ??
    stageData?.registrationPaidAt ??
    stageData?.registration_date ??
    stageData?.registrationDate;
  ...
  const bondDueRaw = body?.bond_due_date ?? body?.bondDueDate;
  if (bondDueRaw !== undefined) {
    const v = String(bondDueRaw || "").trim();
    update.bond_due_date = v ? v.slice(0, 10) : null;
  }
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    if (msg === "UNAUTHENTICATED") return json({ ok: false, error: "Unauthenticated" }, 401);
  ...
  try {
    ...
    const dealRes = await fetchDealByKey(supabase, key);
    if (dealRes.error) {
      console.error("DEALS_DELETE_LOOKUP_ERROR:", dealRes.error);
      return json({ ok: false, where: "deals.lookup", error: dealRes.error.message }, 500);
    }
    if (!dealRes.data) return json({ ok: false, error: "Deal not found" }, 404);
    ...
    // Best-effort cleanup for related rows where foreign keys may not cascade.
    await Promise.allSettled([
      fromTable(supabase, "deal_activity").delete().eq("deal_id", dealId),
      fromTable(supabase, "deal_banks").delete().eq("deal_id", dealId),
      fromTable(supabase, "bank_notes").delete().eq("deal_id", dealId),
    ]);

    const del = await fromTable(supabase, "deals").delete().eq("id", dealId);
    if (del.error) {
      console.error("DEALS_DELETE_ERROR:", del.error);
      return json({ ok: false, where: "deals.delete", error: del.error.message }, 500);
    }

    return json({ ok: true, deleted: true, id: dealId });
  } catch (e: any) {
    console.error("DEALS_DELETE_FATAL:", e);
    return json({ ok: false, error: e?.message || "Server error" }, 500);
  }
}
