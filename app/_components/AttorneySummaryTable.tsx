...
type Row = { attorney: string; count: number; total: number };
...
export default function AttorneySummaryTable(props: { dealsOverride?: any[] } = {}) {
  ...
  const rows = useMemo(() => {
    ...
    for (const d of list) {
      const attorney = pickAttorneyOrFirm(d);
      const amt = dealAmount(d);

      const cur = map.get(attorney) || { attorney, count: 0, total: 0 };
      cur.count += 1;
      cur.total += amt;
      map.set(attorney, cur);
    }

    return Array.from(map.values()).sort(
      (a, b) => (b.total - a.total) || (b.count - a.count) || a.attorney.localeCompare(b.attorney)
    );
  }, [deals]);
  ...
  return (
    <div className="mt-10 rounded-2xl border border-black/10 bg-white p-8 shadow-sm">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <div className="mt-1 text-xs font-semibold text-black/60">
            Deals received from Capital Bonds, grouped by attorney or firm.
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="mt-5 text-sm font-semibold text-black/70">No attorney/firm data yet.</div>
      ) : (
        <div className="mt-5 overflow-hidden rounded-2xl border border-black/10">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.attorney} className="border-t border-black/10 hover:bg-black/[0.02]">
                  <td className="px-4 py-3 font-semibold text-black">{r.attorney}</td>
      ...
      <div className="mt-3 text-[11px] font-semibold text-black/50">
        Note: Unassigned means the deal doesnt have an attorney/firm captured yet.
      </div>
    </div>
  );
}