...
function parseAttorneyText(raw: any) {
  const text = String(raw ?? "").trim();
  if (!text) return { name: "", firm: "", tel: "", email: "" };

  const telMatch = text.match(/Tel:\s*([^|]+?)(?=\s+Email:|$)/i);
  const emailMatch = text.match(/Email:\s*([^\s|]+)\s*$/i);
  const firmMatch = text.match(/\(([^)]+)\)/);
...
export default function MoveStageCards({
  ...
  useEffect(() => {
    ...
    if (stage === "instructed") {
      const instructedAttorney = stageData?.attorney_details && typeof stageData.attorney_details === "object"
        ? stageData.attorney_details
        : parseAttorneyText(
            firstNonEmpty(
              stageData?.attorney,
              src?.instructed_attorney,
              src?.registration_attorney,
              src?.attorney
            )
          );
      ...
      setInstructed((s) => ({
        ...
        bankRefs: { ...parsedRefs, ...bankRefsMap, ...dealBanksRefs, ...bankRefByName },
        attorney: {
          name: firstNonEmpty(instructedAttorney?.name, s.attorney.name),
          firm: firstNonEmpty(instructedAttorney?.firm, s.attorney.firm),
          tel: firstNonEmpty(instructedAttorney?.tel, s.attorney.tel),
          email: firstNonEmpty(instructedAttorney?.email, s.attorney.email),
        },
        estimatedRegDate: String(
    ...
    if (stage === "registrations") {
      const instructedData = pickLatestStageData(src, "instructed") || {};

      const registrationsAttorney =
        stageData?.attorney_details && typeof stageData.attorney_details === "object"
          ? stageData.attorney_details
          : parseAttorneyText(
              firstNonEmpty(
                stageData?.attorney,
      ...
      const instructedAttorney =
        instructedData?.attorney_details && typeof instructedData.attorney_details === "object"
          ? instructedData.attorney_details
          : parseAttorneyText(
              firstNonEmpty(
                instructedData?.attorney,
                src?.instructed_attorney,
                src?.attorney
              )
            );
      ...
      setRegs((s) => ({
        ...
        reference: firstNonEmpty(
          stageData?.registration_attorney_reference,
          stageData?.attorney_ref,
          instructedData?.attorney_ref,
          src?.registration_attorney_reference,
          src?.instructed_reference,
          src?.instructed_attorney_reference,
          s.reference
        ),
        attorney: {
          name: firstNonEmpty(registrationsAttorney?.name, instructedAttorney?.name, s.attorney.name),
          firm: firstNonEmpty(registrationsAttorney?.firm, instructedAttorney?.firm, s.attorney.firm),
          tel: firstNonEmpty(registrationsAttorney?.tel, instructedAttorney?.tel, s.attorney.tel),
          email: firstNonEmpty(registrationsAttorney?.email, instructedAttorney?.email, s.attorney.email),
        },
        registrationDate: firstNonEmpty(
          stageData?.registration_paid_at,
          stageData?.registration_date,
          src?.registration_paid_at,
          s.registrationDate
        ).slice(0, 10),
        paymentDue: firstNonEmpty(
          stageData?.payment_due_date,
          instructedData?.estimated_reg_date,
          instructedData?.instructed_estimated_reg_date,
          src?.payment_due_date,
          src?.estimated_reg_date,
          src?.instructed_estimated_reg_date,
          s.paymentDue
        ).slice(0, 10),
      }));
    }
  ...
  const computedStagePayload = useMemo(() => {
    ...
    if (stage === "registrations") {
      ...
      return {
        ...
        registration_date: regs.registrationDate ? String(regs.registrationDate).slice(0, 10) : null,
        registration_paid_at: regs.registrationDate ? String(regs.registrationDate).slice(0, 10) : null,
        attorney_details: a,
        ...
        <div className="rounded-2xl border border-black/10 bg-white p-3">
          ...
          <div className="mt-3 grid grid-cols-1 gap-3">
            <Field label="Registration date">
              <input
                type="date"
                value={regs.registrationDate}
                onChange={(e) => setRegs((s) => ({ ...s, registrationDate: e.target.value }))}
                className="w-full rounded-2xl border border-black/10 bg-white px-3 py-2 text-xs font-semibold text-black outline-none focus:border-black/30"
              />
            </Field>
