                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-extrabold text-black whitespace-nowrap tabular-nums">{formatZar(s.value)}</span>
                    </div>
                  </button>
                );
              })
...
export default function RegistrationsConsultantBlock(props: { dealsOverride?: any[] } = {}) {
  const hook = useDeals();
  ...
  const error = Array.isArray(props.dealsOverride) ? null : hook.error;
  ...
  const registrationsDeals = useMemo(() => {
    return (deals || []).filter((d: any) => String(d?.stage || "").toLowerCase() === "registrations");
  }, [deals]);

  const totalRegistrations = useMemo(() => {
    return registrationsDeals.reduce((sum: number, d: any) => sum + getAmount(d), 0);
  }, [registrationsDeals]);

  const registrationsByConsultant = useMemo(() => {
    const map = new Map<string, { consultant: string; total: number; count: number }>();

    for (const c of allConsultants) {
      map.set(c, { consultant: c, total: 0, count: 0 });
    }

    for (const d of registrationsDeals || []) {
  ...
  }, [allConsultants, registrationsDeals]);

  const donutSlices = useMemo(() => {
    const positive = registrationsByConsultant
      .filter((x) => x.total > 0)
      .map((x) => ({
        label: x.consultant,
        key: x.consultant,
    ...
    return [
      ...top,
      { label: "Other", key: "__other__", value: otherValue, color: "#9CA3AF" },
    ];
  }, [registrationsByConsultant]);

  return (
    <div className="rounded-2xl border border-black/10 bg-white p-8 shadow-sm">
      <div className="mb-2 text-xl font-extrabold text-black">Registrations Value per Consultant</div>
      <div className="mb-8 text-sm text-black/60">
        {registrationsDeals.length} registrations deal(s)  Total{" "}
        <span className="font-extrabold text-black whitespace-nowrap tabular-nums">{formatZar(totalRegistrations)}</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-10 xl:grid-cols-2">
          ...
          <div className="rounded-2xl border border-black/10 bg-white p-6">
            <div className="mb-4 text-xs font-extrabold text-black">Consultant Leaderboard (All Consultants)</div>

            <div className="overflow-hidden rounded-2xl border border-black/10">
              <table className="w-full text-left text-xs">
                <thead className="bg-black/5">
                  <tr>
                    <th className="px-4 py-3 font-extrabold text-black">Consultant</th>
                    <th className="px-4 py-3 font-extrabold text-black">Registrations</th>
                    <th className="px-4 py-3 text-right font-extrabold text-black whitespace-nowrap tabular-nums">Registrations Value</th>
                  </tr>
                </thead>
                <tbody>
                  {registrationsByConsultant.map((row) => {
                    const pct = totalRegistrations > 0 ? (row.total / totalRegistrations) * 100 : 0;
                    ...
                    return (
                      <tr key={row.consultant} className="border-t border-black/10">
                        <td className="px-4 py-3 font-semibold text-black">
                          ...
                          <div className="mt-2 text-[11px] text-black/50">Share of total registrations:{" "}
                            <span className="font-extrabold text-black">{pct.toFixed(1)}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 align-top font-semibold text-black tabular-nums">{row.count}</td>
                        <td className="px-4 py-3 align-top text-right font-extrabold text-black whitespace-nowrap tabular-nums">
                          {formatZar(row.total)}
                        </td>
                      </tr>
                    );
            ...
            <div className="mt-5 text-xs text-black/50">Colors are stable per consultant (same consultant = same color everywhere).</div>
          </div>
        </div>
      )}
    </div>
