...
export default function DealViewClient({ dealKey }: Props) {
  ...
  const [currentRole, setCurrentRole] = useState<"admin" | "consultant">("consultant");
  ...
  const [bondDueDate, setBondDueDate] = useState("");
  const [bondDueSaving, setBondDueSaving] = useState(false);
  const [bondDueErr, setBondDueErr] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        const res = await fetch("/api/auth/me", { cache: "no-store" });
        const json = await res.json().catch(() => ({}));
        const role = String(json?.user?.role || "").trim().toLowerCase();
        if (alive) setCurrentRole(role === "admin" ? "admin" : "consultant");
      } catch {
        if (alive) setCurrentRole("consultant");
      }
    }
    run();
    return () => {
  ...
  useEffect(() => {
    ...
    setBondDueDate(String((deal as any)?.bond_due_date ?? (deal as any)?.bondDueDate ?? "").slice(0, 10));
  ...
  const showSubmittedBondDue =
    searchParams.get("from") === "submitted" ||
    String((deal as any)?.stage || "").toLowerCase() === "submitted";
  ...
  function previousStageFor(stageRaw: any) {
    const stage = String(stageRaw || "").trim().toLowerCase();
    if (stage === "aip") return "submitted";
    if (stage === "granted") return "aip";
    if (stage === "instructed") return "granted";
    if (stage === "registrations") return "instructed";
    if (stage === "ntu") return "registrations";
    return "";
  }

  async function moveBackOneStage() {
    if (!deal) return;
    const targetStage = previousStageFor((deal as any)?.stage);
    if (!targetStage) {
      setAdminMsg({ type: "err", text: "No previous stage available for this deal." });
      return;
    }
    ...
    try {
      ...
      const res = await fetch("/api/deals/move", {
        ...
        body: JSON.stringify({
          dealId: String((deal as any)?.id || dealKey),
          toStage: targetStage,
          movedBy,
          note: "Admin move: moved back one stage",
        }),
      });
      const json = await res.json().catch(() => ({} as any));
      if (!res.ok || json?.ok === false) {
        throw new Error(json?.error || "Failed to move deal back");
      }
      setAdminMsg({ type: "ok", text: `Moved back to ${targetStage}.` });
      router.refresh();
    } catch (e: any) {
      setAdminMsg({ type: "err", text: e?.message || "Failed to move deal back" });
    } finally {
      setAdminBusy(false);
    }
  }

  async function deleteDealAdmin() {
    if (!deal) return;
    const yes = window.confirm("Delete this deal permanently? This cannot be undone.");
    if (!yes) return;

    setAdminBusy(true);
    setAdminMsg(null);
    try {
      const key = String((deal as any)?.id || dealKey || "").trim();
      const res = await fetch(`/api/deals/${encodeURIComponent(key)}`, { method: "DELETE" });
      const json = await res.json().catch(() => ({} as any));
      if (!res.ok || json?.ok === false) {
      ...
      setAdminMsg({ type: "err", text: e?.message || "Failed to delete deal" });
    } finally {
      setAdminBusy(false);
    }
  }
  ...
  async function saveBondDueDate(next: string, prev: string) {
    if (!dealKey) return;
    setBondDueSaving(true);
    setBondDueErr(null);
    ...
    try {
      const res = await fetch(`/api/deals/${encodeURIComponent(dealKey)}`, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ bond_due_date: next || null }),
      });

      const json = await res.json().catch(() => ({} as any));
      if (!res.ok || json?.ok === false) {
        throw new Error(json?.error || `Failed to update bond due date (${res.status})`);
      }
      ...
      setDeal((prevDeal) =>
        prevDeal
          ? {
              ...prevDeal,
              bond_due_date: next || null,
            }
          : prevDeal
      );
    } catch (e: any) {
      setBondDueErr(e?.message || "Failed to update bond due date");
      setBondDueDate(prev);
    } finally {
      setBondDueSaving(false);
    }
  }
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-6">
      ...
      {isAdmin ? (
        <div className="mt-4 rounded-2xl border border-black/10 bg-white p-5 shadow-sm">
          <div className="text-sm font-extrabold text-black">Admin actions</div>
          <div className="mt-1 text-xs font-semibold text-black/60">
            Admin can move this deal back one stage or delete it.
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              type="button"
              onClick={moveBackOneStage}
              disabled={adminBusy}
            ...
            >
              {adminBusy ? "Saving..." : "Move back one stage"}
            </button>
            <button
              type="button"
              onClick={deleteDealAdmin}
              disabled={adminBusy}
              className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-xs font-extrabold text-red-700 hover:bg-red-100 disabled:opacity-60"
            >
              {adminBusy ? "Saving..." : "Delete deal"}
            </button>
          </div>
          {adminMsg ? (
            <div className={adminMsg.type === "ok" ? "mt-2 text-xs font-semibold text-green-700" : "mt-2 text-xs font-semibold text-red-600"}>
              {adminMsg.text}
            </div>
          ) : null}
        </div>
      ...
      <div className="mt-4 rounded-2xl border border-black/10 bg-white p-5 shadow-sm">
        ...
        {showSubmittedBondDue ? (
          <div className="mt-3 rounded-xl border border-black/10 bg-white p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <div className="text-[11px] font-extrabold text-black/45">Bond Due Date</div>
                <div className="text-sm font-semibold text-black/70">Set or update the due date for this submitted deal.</div>
              </div>
              <div className="flex items-center gap-2">
                <input
                  type="date"
                  value={bondDueDate}
                  onChange={(e) => setBondDueDate(e.target.value)}
                  className="rounded-xl border border-black/10 bg-white px-3 py-2 text-sm font-semibold text-black outline-none focus:border-black/30"
                />
                <button
                  type="button"
                  onClick={() =>
                    saveBondDueDate(
                      String(bondDueDate || "").trim(),
                      String((deal as any)?.bond_due_date ?? (deal as any)?.bondDueDate ?? "").slice(0, 10)
                    )
                  }
                  disabled={bondDueSaving}
                  className="rounded-xl border border-black/10 bg-white px-3 py-2 text-xs font-extrabold text-black hover:border-black/20 disabled:opacity-60"
                >
                  {bondDueSaving ? "Saving..." : "Save"}
                </button>
              </div>
            </div>
            {bondDueErr ? (
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
          {summary
            .filter((s: any) => {
              const currentStage = String((deal as any)?.stage || "").trim().toLowerCase();
              if (s.label === "Estimated Reg Date" && currentStage === "granted") return false;
