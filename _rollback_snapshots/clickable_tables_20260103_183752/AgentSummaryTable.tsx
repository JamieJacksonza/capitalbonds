            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.agent} className="border-t border-black/10 hover:bg-black/[0.02]">
                  <td className="px-4 py-3 font-semibold text-black"><Link href={`/agent/${encodeURIComponent(String(r.agent))}` } className="font-extrabold hover:underline">{r.agent}</Link></td>
