...
function Pill({ text }: { text: string }) {
  return (
    <span className="inline-flex items-center rounded-full border border-black/10 bg-black/[0.03] px-2.5 py-1 text-xs font-extrabold text-black/80">
      {text}
    </span>
  );
}
    ...
    <div className="overflow-hidden rounded-2xl border border-black/10">
      <table className="w-full text-left text-sm">
        <thead className="bg-black/[0.02]">
          <tr className="text-xs font-extrabold text-black/80">
        ...
        <tbody>
          {deals.length === 0 ? (
            <tr className="border-t border-black/10">
              <td className="px-4 py-6 text-sm font-semibold text-black/80" colSpan={8}>
            ...
            deals.map((d) => (
              <tr key={d.id} className="border-t border-black/10 hover:bg-black/[0.02]">
                ...
                <td className="px-4 py-3 text-black/70">{d.submitted}</td>
                <td className="px-4 py-3">
                  <Pill text={d.status} />
                </td>
                <td className="hidden px-4 py-3 text-black/80 lg:table-cell">
                  {d.notes ? d.notes : "—"}
                </td>
                <td className="px-4 py-3">
