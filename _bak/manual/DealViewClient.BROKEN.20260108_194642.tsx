...
async function fetchDealByKey(sb: any, key: string) {
  if (isUuid(key)) {
    const r = await fromTable(sb, "deals").select("*, deal_banks(id, bank_name, bank_notes, attorney, attorney_note, created_at, updated_at), deal_activity(id, action, from_stage, to_stage, note, moved_by, moved_at, created_at)").eq("id", key).maybeSingle();
    if (r.data) return r;
  }
  const r2 = await fromTable(sb, "deals").select("*, deal_banks(id, bank_name, bank_notes, attorney, attorney_note, created_at, updated_at), deal_activity(id, action, from_stage, to_stage, note, moved_by, moved_at, created_at)").eq("deal_code", key).maybeSingle();
  if (r2.data) return r2;

  const r3 = await fromTable(sb, "deals").select("*, deal_banks(id, bank_name, bank_notes, attorney, attorney_note, created_at, updated_at), deal_activity(id, action, from_stage, to_stage, note, moved_by, moved_at, created_at)").eq("id", key).maybeSingle();
  return r3;
}
...
export async function PATCH(req: Request) {
  ...
  const res = await fromTable(supabase, "deals")
    .update(update)
    .eq("id", dealId)
    .select("*, deal_banks(id, bank_name, bank_notes, attorney, attorney_note, created_at, updated_at), deal_activity(id, action, from_stage, to_stage, note, moved_by, moved_at, created_at)")
    .single()
    /* AUTOLOG_DEAL_UPDATES_V1 */
    try {
      const before = await sb.from("deals").select("*, deal_banks(id, bank_name, bank_notes, attorney, attorney_note, created_at, updated_at), deal_activity(id, action, from_stage, to_stage, note, moved_by, moved_at, created_at)").eq("id", dealId).maybeSingle();
