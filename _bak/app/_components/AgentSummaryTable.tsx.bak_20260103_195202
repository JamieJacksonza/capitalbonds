...
type Row = { agent: string; count: number; total: number };

export default function AgentSummaryTable(props: { dealsOverride?: any[] } = {}) {
  const hook = useDeals();
  ...
  const rows = useMemo(() => {
    ...
    for (const d of list) {
      const agent = pickAgent(d);
      const amt = dealAmount(d);

      const cur = map.get(agent) || { agent, count: 0, total: 0 };
      cur.count += 1;
      cur.total += amt;
      map.set(agent, cur);
    }
    ...
    return Array.from(map.values()).sort(
      (a, b) => (b.total - a.total) || (b.count - a.count) || a.agent.localeCompare(b.agent)
    );
  }, [deals]);
  ...
  return (
    <div className="mt-10 rounded-2xl border border-black/10 bg-white p-8 shadow-sm">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <div className="text-lg font-extrabold text-black">Agent Summary</div>
          <div className="mt-1 text-xs font-semibold text-black/60">
            Deals received from Capital Bonds, grouped by agent.
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="mt-5 text-sm font-semibold text-black/70">No agent data yet.</div>
      ) : (
        <div className="mt-5 overflow-hidden rounded-2xl border border-black/10">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.agent} className="border-t border-black/10 hover:bg-black/[0.02]">
                  <td className="px-4 py-3 font-semibold text-black">{r.agent}</td>
                  <td className="px-4 py-3 font-extrabold text-black">{r.count}</td>
                  <td className="px-4 py-3 text-right font-extrabold text-black whitespace-nowrap">{moneyZar(r.total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="mt-3 text-[11px] font-semibold text-black/50">
        Note: Unassigned means the deal doesnt have an agent captured yet.
      </div>
    </div>
  );
}