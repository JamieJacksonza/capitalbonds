...
# Write a fresh, working component
@"
"use client";
...
export default function StatusBankNotesCard({ dealKey, amountZar, stage }: Props) {
  ...
  const stageKey = useMemo(() => String(stage ?? "submitted").trim().toLowerCase() || "submitted", [stage]);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const url = `/api/deals/${encodeURIComponent(key)}/bank-notes`;
  ...
  async function save() {
    ...
    try {
      // Try PATCH first (common for updates)
      ...
      // If PATCH not allowed, try POST as fallback
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-[11px] font-semibold text-black/60">
            Stage: <span className="font-extrabold text-black/80">{stageKey}</span>
            {amountLine ? <>  Amount: <span className="font-extrabold text-black/80">{amountLine}</span></> : null}
          </div>
        </div>
      ...
      {msg ? (
        <div
          className={
            ...
            (msg.type === "ok" ? "border-black/10 bg-black/[0.02] text-black" : "border-black/20 bg-black/[0.03] text-black")
          }
        >
          {msg.text}
        </div>
          ...
          <div key={b.key} className="rounded-2xl border border-black/10 bg-white p-3">
            ...
            <textarea
              ...
              onChange={(e) =>
                setRows((prev) => ({
                  ...prev,
                  [b.key]: { ...(prev[b.key] || { bank_name: b.key, note: "" }), note: e.target.value },
                }))
              }
