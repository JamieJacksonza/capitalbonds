param(
  [string]$File = ".\app\api\deals\[id]\bank-notes\route.ts"
)
...
# Replace the whole file with an upsert-capable version (keeps your existing behavior but adds inserts)
$new = @"
...
async function handler(req: Request, ctx: { params: { id: string } }) {
  try {
    ...
    for (const item of list) {
      ...
      // 2) No id -> ensure bank_name exists then upsert (create row if missing)
      ...
      if (!existing.error && existing.data?.id) {
        ...
        if (ins.error) {
          return new Response(JSON.stringify({ ok: false, error: ins.error.message }), { status: 500 });
        }
...
Write-Host "Updated (upsert-enabled):" -ForegroundColor Green
