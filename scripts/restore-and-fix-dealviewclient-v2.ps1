...
# Find newest backup by NAME only (avoid [id] wildcard issues)
$bak = Get-ChildItem -LiteralPath ".\_bak" -Recurse -File -ErrorAction SilentlyContinue |
  Where-Object { $_.Name -like "DealViewClient.tsx*.bak" } |
  Sort-Object LastWriteTime -Descending |
  Select-Object -First 1

if (-not $bak) {
  throw "No backups found anywhere under .\_bak for DealViewClient.tsx*.bak"
}
...
# Safe patch: allow fallback reassignment
$raw = Get-Content -LiteralPath $file -Raw
if ([string]::IsNullOrWhiteSpace($raw)) { throw "Restored file is empty (unexpected)." }

$new = $raw

# join any accidental multiline className strings (harmless if none)
$pattern = 'className="([^"]*)\r?\n\s*([^"]*)"'
$loops = 0
do {
  $loops++
...
Write-Host "`nSANITY CHECK (first 8 lines):" -ForegroundColor Cyan
Get-Content -LiteralPath $file | Select-Object -First 8
